...
Deeper networks exhibit a degradation problem: the more layers are added, the higher
the training/test error because the model's fitted function is more variable.

A group of consecutive layers is referred to as a block. In the paper, the block is composed
of 3 layers: 1x1 convolutions, 3x3 convolutions and 1x1 convolutions. Instead of hoping that
the block will fit an underlying mapping $H(x)$, we make the block directly fit a residual
mapping $F(x) = H(x) - x$, where the underlying mapping becomes $H(x) = F(x) + x$. $F(x)$
is a layer output (linear combination, convolution, etc) and $+ x$ refers to adding the block
input to the block output.

The dimensions of $F(x)$ and $x$ must the same. If not, a linear projection of $x$ is used.
The element wise addition of an input and output convolutional layer is done channel by channel.
If the channel counts aren't the same, zero padding is used (adding no extra parameters) or 1x1
convolutions are used (adding more paraemters). 1x1 convolutions produce better results. If the
dimensions aren't the same, the element wise addition is done is a strided way to downsample the input.
