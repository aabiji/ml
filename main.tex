...
\item How is model training done on GPU on a low level?
  \item What is CUDA?
  \item How is model training is done on multiple GPUs?
...
\item caling laws are indeed magical, but blindly adding more parameters is a faulty approach. What if SOTA models can be much, much smaller?
\item What would a generalist autonomous robot need to do a fast food job?
\end{itemize}
...
The input sequence is first converted into a sequence of tokens. For example, \href{https://en.wikipedia.org/wiki/Byte-pair_encoding}{Byte-pair encoding} can be used to convert text into a sequence of tokens that are word fragments. These tokens are then processed by the Transformer's \href{https://machinelearningmastery.com/wp-content/uploads/2021/08/attention_research_1.png}{encoder and decoder}. Residual connections help information propagate through the network, and a fixed positional encoding function is used to help the network order its input embeddings.

Three different variants of multi-head attention are used. Self-attention, where the query, keys and values are derived from the same input sequence, is used to attend to the entire input sequence. Masked self-attention, where a causal mask ($0$ below the diagonal, $-\infty$ on and above the diagonal) is added to the similarity matrix before softmax is applied, preventing each position from attending to future tokens. Cross attention, where $Q$ is from the decoder layer and $K, V$ are from the very last encoder layer, allows each output symbol to attend to all input symbols at once.

$Q \in \mathbb{R}^{n_q \times d_k}$, $K \in \mathbb{R}^{n_k \times d_k}$, $V \in \mathbb{R}^{n_k \times d_v}$, where $d_k$ and $d_v$ are embedding dimensions of the keys and values. Each attention head computes how similar each query is to each key and stores that in a similarity matrix. The similarity matrix is scaled by $1 / \sqrt{d_k}$ which prevents the dot product from growing excessively large as $d_k$ increases, which would cause softmax to become overly peaked, resulting in tiny gradients. Each row in the similarity matrix is used to weigh how relevant each value is to the query.

The input embeddings are projected into a smaller vector subspace $h$ times, producing $h$ different representations of $Q, K, V$. This allows each head to compute attention using a different learned notion of similarity.
...
The feed forward network is composed of two layers, $\text{ReLU}(\mathbf{x} W_1 + b_1) W_2 + b_2$, which process each embedding independently in parallel after the multi-head attention has incorporated necessary context on each one.

There is still room for improvement. The time complexity for computing attention is still quadratic...
What exactly are embeddings? What are their geometric properties?
