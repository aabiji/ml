...
Prior to the use of Transformers, RNNs that followed an encoder-decoder structure were used to learn sequence transduction problems, such as machine translation. In \href{https://arxiv.org/abs/1409.0473}{Neural Machine Translation By Jointly Learning to Align and Translate}, the concept of using attention to better predict an output sequence was introduced. Attention is a mechanism for encoding how relevant each input symbol is in predicting the next output symbol, relative to the other input symbols. This is done by introducing a context vector which is just a weighted sum of vectors (in the paper, vectors are the encoder's hidden states, which are representations of the input sequence up to each input symbol position). Instead of trying to encode the entire input sequence into the context vector, an approach which falls apart as the input sequence length increases, certain symbols in the input sequence are emphasized more than others through the way that they're weighted.

The problem with using the model described in \textit{Neural Machine Translation By Jointly Learning to Align and Translate} isn't necessarily in the use of RNNs, but in the way that attention is computed; it's extremely inefficient. For the decoder to predict $n$ output symbols from $m$ input symbols, $m$ steps must be done sequentially in the encoder and $n$ steps must be done sequentially in the decoder. To calculate the context vector for each step in the decoder, you need to do a weighted sum of all $m$ encoder hidden states. To compute the associated weights used for the encoder hidden states, \textit{another model} is used to compute an alignment score between the current decoder hidden state and each encoder hidden state, which takes another $m$ steps. Therefore, the time complexity for generating the output sequence is $O(m + n * m^2)$.

- What do Query, Key and Value refer to?
