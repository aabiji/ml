...
\item Can one visualize the parameter space of a model? What are the techniques for visualizing a high dimensional space in 3d?
...
\item Does local minima become rarer in higher dimensional space? If so, why is SGD effective if its main selling point is using a random walk to avoid local minima?
\item 3D classification models
\item \href{https://suzyahyah.github.io/machine%20learning/2026/06/27/trouble-with-time-series.html}{The Unreasonable Difficulty of Time Series Forecasting}
\item Robot that's general enough to work at a fast food restaurant (ex: Tim Hortons team member)
\item What if we focused on efficiency rather than scale? Scaling laws are indeed magical, but blindly adding more parameters is a faulty approach. What if SOTA llms can be much, much smaller (small enough to run on a tiny gpu cluster).
\end{itemize}
...
Reccurent Neural Networks are models designed for processing sequences of data. Depending on how it's designed, an RNN can predict a sequence from a sequence, a sequence from a value or a value from a sequence. Here, sequence to sequence prediction will be used to \href{https://www.mdpi.com/information/information-15-00517/article_deploy/html/images/information-15-00517-g001-550.jpg}{explain how RNNs work}.
...
Backpropagation through time (BPTT) is an adaptation of the backpropagation algorithm for RNNs. The main difference is that the derivative of the loss with respect to $W_x$, $W_o$ and $W_h$ must be computed considering each symbol in the input sequence.
The jump made at $(*)$, is because $\frac{\partial h_t}{\partial a_h}$ recursively depends on previous hidden blocks. As $t - k$ increases, $W_k^{\top}$ becomes increasingly larger and numerically unstable. So, $t - k \le c$, where $c$ is the window size, the upper bound on the number of symbols the RNN remembers at once.

\begin{align*}
\frac{\partial L(o, y)}{\partial W_o}
&= \sum_{t = 1}^{T} \frac{\partial l_t(o_t, y_t)}{\partial W_o} \\
&= \sum_{t = 1}^{T} \frac{\partial l_t(o_t, y_t)}{\partial o_t} \frac{\partial o_t}{\partial a_o} \frac{\partial a_o}{\partial W_o} \\
&= \sum_{t = 1}^{T} \frac{\partial l_t(o_t, y_t)}{\partial o_t} \frac{\partial o_t}{\partial a_o} h_t \\
\end{align*}

\begin{align*}
\frac{\partial L(o, y)}{\partial W_x}
&= \sum_{t = 1}^{T} \frac{\partial l_t(o_t, y_t)}{\partial W_o} \\
&= \sum_{t = 1}^{T} \frac{\partial l_t(o_t, y_t)}{\partial o_t} \frac{\partial o_t}{\partial a_o} \frac{\partial a_o}{\partial h_t} \frac{\partial h_t}{\partial a_h} \frac{\partial a_h}{\partial W_x} \\
&= \sum_{t = 1}^{T} \frac{\partial l_t(o_t, y_t)}{\partial o_t} \frac{\partial o_t}{\partial a_o} W_o \frac{\partial h_t}{\partial a_h} x_t \\
&= \sum_{t = 1}^{T} \frac{\partial l_t(o_t, y_t)}{\partial o_t} \frac{\partial o_t}{\partial a_o} W_o \sum_{k=1}^{t} (W_k^{\top})^{t - k} x_k \tag{*}
\end{align*}

\begin{align*}
\frac{\partial L(o, y)}{\partial W_h}
&= \sum_{t = 1}^{T} \frac{\partial l_t(o_t, y_t)}{\partial W_o} \\
&= \sum_{t = 1}^{T} \frac{\partial l_t(o_t, y_t)}{\partial o_t} \frac{\partial o_t}{\partial a_o} \frac{\partial a_o}{\partial h_t} \frac{\partial h_t}{\partial a_h} \frac{\partial a_h}{\partial W_x} \\
&= \sum_{t = 1}^{T} \frac{\partial l_t(o_t, y_t)}{\partial o_t} \frac{\partial o_t}{\partial a_o} W_o \frac{\partial h_t}{\partial a_h} h_{t - 1} \\
&= \sum_{t = 1}^{T} \frac{\partial l_t(o_t, y_t)}{\partial o_t} \frac{\partial o_t}{\partial a_o} W_o \sum_{k=1}^{t} (W_k^{\top})^{t - k} h_k \tag{*}
\end{align*}
...
\href{https://arxiv.org/abs/1409.0473}{Neural Machine Translation By Jointly Learning to Align and Translate} introduced the concept of \textit{attention} to improve the way that RNN encoder-decoder models process sequences. By weighing certain input symbols more than others in the context vectors used by the decoder, the model can focus on the most relevant parts of the input sequence needed to predict the next output symbol. However, the way that the paper computes attention is extremely inefficient. Computing the context vector needed for each decoder step requires summing over every encoder hidden state, resulting in a quadratic time complexity. \href{https://arxiv.org/abs/1706.03762}{Attention Is All You Need} introduces the Transformer, a model architecture that performs better than the RNN models used previously while addressing their inefficiencies.

The input sequence is first converted into a sequence of tokens. For example, \href{https://en.wikipedia.org/wiki/Byte-pair_encoding}{Byte-pair encoding} is used to convert text into a sequence of tokens that are word fragments. Transformers are split into \href{https://machinelearningmastery.com/wp-content/uploads/2021/08/attention_research_1.png}{an encoder-decoder}. The encoder only needs to be ran once, while the decoder is ran once for each output token. Residual connections help information propagate through the network, and a fixed positional encoding function is used to help the network order its input embeddings.

Three different variants of multi-head attention are used. Self attention, where $Q = K = V$, is used to attend to the entire input sequence. Masked self attention, where a lower triangular matrix ($-\inf$ on the upper half, $0$ on the lower half) is added to the similarity matrix before softmax is applied. This ensures that only previous inputs are attended to. Cross attention, where $Q$ is from the very last encoder layer and $K, V$ are the decoder, allows each output symbol to attend to all input symbols at once.

$Q \in \mathbb{R}^{n \times d_k}$, $K \in \mathbb{R}^{n \times d_k}$, $V \in \mathbb{R}^{n \times d_v}$, where $n$ is the number of embeddings and $d_k$ and $d_v$ are embedding dimensions. Each attention head computes how similar each query is to each key and stores that in a similarity matrix. The similarity matrix is scaled by $1 / d_k$ to prevent variance from growing as a result of the dot product. Each row in the similarity matrix is used to weigh how relevant each value is to the query. $Q, K, V$ are projected into a smaller subspace $h$ times, extracting $h$ key features from the input, and attention is performed in parallel for each feature.
...
The feed forward network is composed of two layers, $\text{ReLU}(\mathbf{x} W_1 + b_1) W_2 + b_2$, which process each embedding individually in parallel after the multi-head attention has capture necessary context on each one.
