...
\section{Questions}
\begin{itemize}
\item A massive problem in deep learning now is that we don't have any clue how these neural networks actually work once trained.
...
(2) To rotate by 180 degrees, F is flipped vertically then horizontally. The gradient dilated by the current layer's stride and zero-padded by the current layer's kernel size minus one. After the convolution, the gradient should have its padding stripped.

\begin{align*}
\frac{\partial L}{\partial F} &= \text{convolution}\left[X, \frac{\partial l_i}{\partial f_k}\right] \tag{1} \\\\
\frac{\partial L}{\partial X} &= \text{convolution}\left[\frac{\partial l_i}{\partial f_k}, \text{rot180}[F]\right] \tag{2}\\
\frac{\partial L}{\partial B_k} &= \sum_{y=1}^{H} \sum_{x=1}^{W} \frac{\partial l_i}{\partial f_{k,y,x}} \tag{3}
\end{align*}
...
\begin{itemize}
  ...
  \item \textbf{Data Augmentation}: Generate different variations of the input data (add synonyms to text, blur images, filter audio, etc) in order to teach the model to be indifferent to irrelevant data transformations.
...
Batch normalization allows the model to converge in fewer steps, reduces the need for dropout by acting as a regularizer and allows for the use of higher learning rates without risk of divergence.

The change in distribution of hidden units is referred to as covariate shift.

\section{Residual Networks}
Deeper networks exhibit a degradation problem: the more layers are added, the higher the training/test error because the model's fitted function is more variable.

A group of consecutive layers is referred to as a block. In the paper, the block is composed of 3 layers: 1x1 convolutions, 3x3 convolutions and 1x1 convolutions. Instead of hoping that the block will fit an underlying mapping $H(x)$, we make the block directly fit a residual
mapping $F(x) = H(x) - x$, where the underlying mapping becomes $H(x) = F(x) + x$. $F(x)$ is a layer output (linear combination, convolution, etc) and $+ x$ refers to adding the block input to the block output.

The dimensions of $F(x)$ and $x$ must the same. If not, a linear projection of $x$ is used. The element wise addition of an input and output convolutional layer is done channel by channel. If the channel counts aren't the same, zero padding
is used (adding no extra parameters) or 1x1 convolutions are used (adding more paraemters). 1x1 convolutions produce better results. If the dimensions aren't the same, the element wise addition is done is a strided way to downsample the input.
