...
Attention is a mechanism for encoding how relevant each input symbol is in predicting the next output symbol relative to the other input symbols. Instead of trying to encode the entire input sequence into a fixed length vector, an approach which falls apart as the input sequence length increases, certain symbols in the input sequence are emphasized more than others. This ensures that the encoded vector only contains a limited number of highly relevant symbols.
